...
def build_features(
    ...
    with_dropped_features: bool,
    with_autoregressive_features: bool,
) -> pd.DataFrame:
    df_feat = processor.apply_feature_engineering_pipeline(
        ...
        with_autoregressive_features=with_autoregressive_features,
    )

    if with_dropped_features:
...
def plot_day(
    ...
    fig.suptitle(f"XGBoost Forecast vs Actual — {date}", fontsize=14, fontweight="bold")
...
async def walk_forward_month(
    ...
    output_dir: str = "pipeline_output_xgboost/walk_forward",
    ...
    for day in days:
        ...
        df_feat = build_features(
            ...
            fit_scaler=False,
            start_dt=start_dt,
            with_dropped_features=with_dropped_features,
            with_autoregressive_features=with_autoregressive_features,
        )
        ...
        preds = model.forecast(steps=steps, exog_forecast=df_feat[exog_cols])
        ...
        print(f"  Processed {steps} steps for direct forecasting.\n")
    ...
    print(f"{month_name} {year} — XGBOOST MONTHLY SUMMARY")
    ...
    fig.suptitle(
        f"XGBoost Walk-Forward — {month_name.capitalize()} {year}  |  Monthly MAPE: {monthly_metrics['MAPE']:.2f}%  RMSE: {monthly_metrics['RMSE']:.1f} MW",
        fontsize=13,
        fontweight="bold",
    )
    ...
    ax2 = axes[1]
    day_labels = [m["date"][5:] for m in all_metrics]
    ...
    async def main():
        ...
        async for session in timescale_database.get_session():
            ...
            await walk_forward_month(
                year=2026,
                month=4,
                model_path="pipeline_output_xgboost/models/xgboost_scaled_with_dropped_features.pkl",
                repository=repository,
                processor=processor,
                train_start=train_start,
                train_end=train_end,
                output_dir="pipeline_output_xgboost/walk_forward/xgboost_scaled_with_dropped_features",
