...
from forecasting.load.prediction_visualizer import LoadForecastingPredictionVisualizer
from forecasting.load.repository import LoadForecastingRepository
from forecasting.load.xgboost import LoadForecastingXGBoost


class LoadForecastingXGBoostPredictionService:
    def __init__(
        self,
        ...
        with_autoregressive_features: bool = True,
    ):
        ...
        self.with_autoregressive_features = with_autoregressive_features
    ...
    async def _fetch_and_process_data(
        ...
        processed_df = self.processor.apply_feature_engineering_pipeline(
            ...
            with_autoregressive_features=self.with_autoregressive_features,
        )
    ...
    async def fit_and_save(
        self,
        model_filename: str = "xgboost_base.pkl",
        **fit_kwargs,
    ) -> str:
        print(
        ...
        exog_cols = [col for col in train_processed.columns if col != "load_mw"]

        print("Training XGBoost Regressor on GPU...")
        model = LoadForecastingXGBoost()
        model.train(
    ...
    async def optimize_and_save_model(
        self, model_filename: str = "xgboost_optimized.pkl"
    ):
        print(
            f"Fetching training data ({self.start_date_train} to {self.end_date_train})..."
        )
        ...
        print("Optimizing and Training XGBoost Model via Cross-Validation...")
        model = LoadForecastingXGBoost()
    ...
    async def predict_range(
        ...
        model_filename: str = "xgboost_optimized.pkl",
    ) -> dict:
        model_path = os.path.join(self.models_dir, model_filename)
        ...
        print(f"Fetching inference features ({start_date} to {end_date})...")
        ...
        self.visualizer.plot_forecast_vs_actual(
            y_true,  # type: ignore
            predictions,
            title=f"XGBoost Forecast vs Actual: {start_date.date()} to {end_date.date()}",
    ...
    async def walk_forward_validate(
        self, test_days: int, model_filename: str = "xgboost_base.pkl"
    ) -> dict:
        model_path = os.path.join(self.models_dir, model_filename)
        if not os.path.exists(model_path):
            raise FileNotFoundError(
        ...
        exog_cols = [col for col in test_processed.columns if col != "load_mw"]
        ...
        for day in range(test_days):
            ...
            pred_chunk = model.forecast(steps=steps_per_day, exog_forecast=current_exog)

            all_predictions.extend(pred_chunk)
            all_actuals.extend(current_actuals)

            print(f"Processed Day {day + 1}/{test_days}")
        ...
        with open(metrics_path, "w") as f:
            json.dump(
                {
                    "model_hyperparameters": model.model_params,
                    "test_days": test_days,
                    "metrics": metrics,
                },
                f,
        ...
        self.visualizer.plot_forecast_vs_actual(
            all_actuals,
            all_predictions,
            title=f"Walk-Forward Validation ({test_days} Days) - XGBoost",
